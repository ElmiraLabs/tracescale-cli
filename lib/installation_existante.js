...
export function installationExistante(dossierCible) {
  ...
  // tracescale#1223 : en natif, la version installée est celle de CE QUI
  // TOURNE — `apps/api/build/package.json`, déposé par l'artefact API —
  // jamais celle du checkout, que ce CLI rafraîchit lui-même avant de
  // déléguer (sinon un échec de mise à jour laisserait un checkout N+1 avec
  // un build N, et le CLI répondrait « déjà à jour »). Docker : le checkout
  // reste la seule source (rien ne tourne depuis build/).
  const cheminsVersion =
    cible === 'natif'
      ? [join(dossierCible, 'apps/api/build/package.json'), join(dossierCible, 'package.json')]
      : [join(dossierCible, 'package.json')]
  const version = versionDepuisPackageJson(cheminsVersion)

  return { type, cible, version }
}

function versionDepuisPackageJson(chemins) {
  for (const chemin of chemins) {
    try {
      const pkg = JSON.parse(readFileSync(chemin, 'utf8'))
      if (typeof pkg.version === 'string' && pkg.version) return pkg.version
    } catch {
      // Fichier absent ou illisible : chemin suivant.
    }
  }
  return null
}
