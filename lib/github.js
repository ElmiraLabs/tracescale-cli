...
export async function telechargerManifeste(jeton, assets) {
  ...
  if (!asset) {
    throw new Error(
      `Manifeste d'empreintes « ${NOM_ASSET_MANIFESTE} » introuvable sur cette release — publication par la CI encore en cours (réessayer dans quelques minutes) ou release antérieure à tracescale#1172. Aucun fichier de cette release ne peut être installé sans vérification.`
    )
  }
...
export async function telechargerEtExtraire(release, jeton, dossierCible) {
  ...
  if (!assetSource) {
    throw new Error(
      `Archive source « ${NOM_ASSET_SOURCE} » introuvable sur la release ${release.tag} — soit la publication par la CI est encore en cours (réessayer dans quelques minutes), soit la release date d'avant tracescale#1172 et une plus récente est nécessaire.`
    )
  }
